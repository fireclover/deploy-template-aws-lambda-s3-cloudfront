 ...
 */
export class DynamicSite extends Construct {
  constructor(parent: Stack, name: string, props: DynamicSiteProps) {
    super(parent, name);
    ...
    new CfnOutput(this, 'ApiLambdaFunction', { value: lambdaFunction.functionArn });
    ...
    const CFfunction = folderRedirects 
      ? 'function handler(event) { \
        ...
        : props.isSpa // FOR SPAs - subdirectory level where index.html is located.
          ? 'var level = 0; \
          var regexExpr = /^\/.+(\.\w+$)/; \
          function handler(event) { \
              var request = event.request; \
